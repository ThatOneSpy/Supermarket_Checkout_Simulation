...
public class JavaMarketDriver {
	...
	public static void main(String[] args) {
		...
		// Tracks turn around.. method in CustomerCreator
		int turnaround;
		...
		if (numCustomers == 0) {
			...
			for (int i = 0; i < numCustomers; i++) {
				...
				if ((queueA.size() <= queueB.size() && queueA.size() <= queueC.size())) {
					if (!queueA.isEmpty()) {
						if (queueA.size() > 1) {
							bWait = create.getWait(queueA.getLast(), currentTime);
						} else if (queueA.size() == 1) {
							bWait = create.getWait(queueA.getFirst(), currentTime);
						}
					...
					System.out.println(
							"Customer " + b.getCustomerId() + " entered Queue A with a wait of " + bWait + " minutes.");
					...
					if (!queueB.isEmpty()) {
						if (queueB.size() > 1) {
							bWait = create.getWait(queueB.getLast(), currentTime);
						} else if (queueB.size() == 1) {
							bWait = create.getWait(queueB.getFirst(), currentTime);
						}
						create.resetFinishTime(b, bWait);
						create.resetServeTime(b, bWait);
					...
					System.out.println(b.toString());
					System.out.println(
							"Customer " + b.getCustomerId() + " entered Queue B with a wait of " + bWait + " minutes.");
				}

				else {
					if (!queueC.isEmpty()) {
						if (queueC.size() > 1) {
							bWait = create.getWait(queueC.getLast(), currentTime);
						} else if (queueC.size() == 1) {
							bWait = create.getWait(queueC.getFirst(), currentTime);
						}
						create.resetFinishTime(b, bWait);
						create.resetServeTime(b, bWait);
					...
					System.out.println(b.toString());
					System.out.println(
							"Customer " + b.getCustomerId() + " entered Queue C with a wait of " + bWait + " minutes.");
				}
				a = b;
			}

			System.out.println("Average wait: " + waitAvg(waitList, (numCustomers + 1)));
			System.out.println("Total time checkouts were not in use: " + create.noUse);

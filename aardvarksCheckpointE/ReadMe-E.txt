Java Market Checkpoint E (Aardvarks)

++ New addition of phpMyAdmin database integration that records the data from the program and saves it to a database.
++ The database records 3 different types of data and all data recorded is timestamped with the data and time the data is saved.
	1. Input data (testdata Table)
	2. Customer data (customer Table)
	3. Output data (results Table)

***In-order for the use of the program to run the user is required to set up the database on the user's end***

	There is a SQL script included that is required to be imported into phpMyAdmin to create the database and it's table.
	Create a user with username and password of "DBUser". Use localhost for the host name.
	The user will also need to connect a MySQL JDBC database jar file to the package. 

++ SupermarketDB Class: This has has several methods that interact with the database.
The class has 3 methods that interact with the connection between the database and the program. (Connecting, Disconnecting, and Verification)
The class also 3 has methods that direct the data from the program to the database for saving.
